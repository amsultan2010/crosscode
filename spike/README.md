...
No network, no service, no websocket, no timers, just a virtual clock so the ~10s hot
...
3-way merge works. Guard G3 proves this survives `git gc --prune=now`, because the base of
...
conflict, its own unpublished edit is still pending, and the moment its debounce fires,
...
which is defensible, because losing an edit is worse than a stale file, but it does mean a
...
- Two replicas only. Three-way convergence (PLAN.md phase 4 wants three daemons) is
  not proven, and the shadow is a single "agreed state", and with N peers it is not
