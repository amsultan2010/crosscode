...
| Skills | 0 | 1 | **1** |
| DB tables | 24 | 8 | **8**: the six below, plus `device_codes` and `terms_acceptances` |
| Source lines | ~13,700 | ~8,650 | **~3,500** |
...
Four of the five rows are at target. Source lines sit at 8,650, up from 7,850, because
sign-in, terms acceptance, and the `HEAD`-move handling all landed. `packages/protocol` is
296 lines and holds nothing transaction-shaped. Closing the last row means deleting from
`apps/service` (2,714) and `apps/daemon` (2,242), and nothing on this page authorises that
yet.
...
no lifecycle, no accept/reject. `op` is `modify` or `delete`, and without it a delete is
...
The sender advances its shadow on send. Binaries are never merged, so concurrent binary edits
...
Presence is in-memory in the websocket gateway. Two more support them: `device_codes` holds
a hashed device code and the user code the browser binds to a session, short-lived and
single-use, and `terms_acceptances` records the accepted version of the terms and privacy
notice per account.
...
**Done:** 1 (strip) · 2 (merge core proven) · 3 (protocol + service) · 4 (daemon).
**Mostly done:** 5, 6, each with its verify line outstanding. **In progress:** 7.

A box is ticked when the thing is on `main` and its verify line was actually run. It is not
ticked because the design is agreed or the code is written on a branch somewhere. What is
left open below is what a user would notice, so keep the bar there.
...
- [x] 6-table migration, RLS from day one (`002` adds `device_codes`, `003` adds `terms_acceptances`)
- [x] GitHub OAuth: routes, table and `/device` page. `start` gets a code from
      `POST /v1/auth/github/device`, `/device/token` polls, and the page binds the code to
      a Supabase session.
...
- [ ] **Verify:** fresh machine, link to syncing in under 60s, typing only the two given
      lines. The 60-second figure is a target; no page prints it as a measurement.
...
- [x] Claude Code + Codex hooks firing before file edits. `start` installs
      ...
      path. An upgrade from 0.1.0 rewrites that entry in place rather than leaving it and
      adding a second.
...
### 7: Docs + harden
- [x] Rewrite `README.md` and `docs/{architecture,protocol,onboarding-contracts,mcp-clients,install-prompt}.md`, all of which described the deleted architecture
...
- Three-way convergence is proven for three peers (`packages/sync/src/three-way.test.ts`, `apps/daemon/src/sync-daemon.test.ts`); a single shadow costs an extra merge, never a wrong answer
- Under sustained typing a hot file defers up to `MAX_DEFERRALS` (20), then the write is forced with a backup ref
- **Every migration must carry its own grant to the runtime role.** `002` carries its own
  grant and health fails on an unreadable table, but the next migration has to remember the
  same thing, and only a convention says it will
- **`git pull` can refuse where the content is identical, by design.** Alice commits and
  pushes what she and Bob both hold uncommitted; Bob's `git pull` refuses with "your local
  changes would be overwritten", even though the bytes match. Making it succeed would mean
  touching Bob's working tree to line it up with a commit, which is exactly the magic this
  product refuses. What ships is the noticing: the daemon rebases the shadow and tells
  Bob's agent, which can clear the way itself
