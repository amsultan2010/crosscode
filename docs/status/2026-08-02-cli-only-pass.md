...
2. **Auth: `crosscode login` becomes a browser flow** with a loopback callback and a
   `state` check, keeping the existing email/password path as the headless path for
   agents and CI.
...
`"node": ">=24"`, so pnpm warns and proceeds. Pre-existing, unrelated to this work.
...
**Cross-workstream seam verified by hand after the merge.** It is the one thing neither
...
(−2,078 lines). `esbuild` stayed, because Vite needs it.
...
rather than deleted, because Contracts A (pairing) and C (personal workspace) still
describe live backend behavior. Only the dashboard-facing Contract D went away.

## Not fully completed

Nothing here blocks the build or the tests. These are the gaps.
...
default", and there was no production domain at the time, so the workstream chose to fail fast
rather than guess. Correct call, but it meant bare `crosscode login` did not work out of the
box until a domain existed and the default was filled in.

**Correction (2026-08-04):** done. `apps/daemon/src/hosted.ts` now compiles in
`https://www.getcrosscode.dev` as `DEFAULT_WEB_URL`, so `resolveWebUrl()` no longer throws and
`WEB_URL_REQUIRED` is unreachable.
...
set it, but the name is now wrong. Worth deprecating in favour of `CROSSCODE_WEB_URL`.

**Correction (2026-08-04):** deprecated. Reading it warns once on stderr and points at
`CROSSCODE_WEB_URL`.
...
against a deployed `/auth/cli.html` and completed a real Supabase sign-in, because at the
time there was no deployment to run it against. First real integration test once a domain
exists.

**Correction (2026-08-04):** `https://www.getcrosscode.dev/auth/cli.html` is deployed and
returns 200. The end-to-end run is now possible and still has not been done.

### Untouched by design

- **Billing enforcement.** The `assertSeatCapAvailable` / `assertSemanticReviewCallAvailable` /
  `assertPlanAllowsAutonomyTier` helpers still are not wired into their call sites. That is
  a pre-existing Phase 10 gap, not something this pass touched.
