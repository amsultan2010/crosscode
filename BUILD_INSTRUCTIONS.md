...
Crosscode now has a tested local safety core and a working multi-replica coordination path: real daemons exchange live proposals, tasks, claims, handoffs, and validation results through an authenticated PostgreSQL-backed service, deterministically classify conflicts, optionally auto-apply low-risk proposals under an explicit committed policy, gate publish on validation, and are reachable from any MCP client. The daemon remains the sole local authority; the CLI and MCP entry point communicate through its authenticated loopback API. AI semantic review now runs against a real, non-mock reviewer, `AgentDelegatedReviewer`, that delegates review to the workspace member's own connected MCP agent instead of calling an external AI provider (Phase 6). A Google-Docs-style live web dashboard with browser accounts is planned as Phase 8 (see section 20) — sequenced after Phase 6, intentionally not part of this repository yet, but a committed future milestone rather than a rejected feature; see section 1's non-goals for the current release boundary.
...
- A standards-compliant MCP stdio server (`@modelcontextprotocol/sdk`) exposing all fourteen section 13 coordination tools with JSON-Schema tool discovery, backed end-to-end by the daemon HTTP client, with documented Claude Code/Codex CLI/OpenCode configurations (`docs/mcp-clients.md`) and automated coverage for `crosscode run -- <tool>` exit-code/argument pass-through.
...
- Phase 6: provider-neutral AI semantic review in `packages/core` (types, strict output schema, redaction/bundle construction, prompt-injection-resistant request construction, policy gates) wired into the daemon's proposal accept flow as a non-authoritative, dependency-injected reviewer, with a `MockSemanticReviewer` test double for unit coverage and a real, non-mock, non-external-provider reviewer implementation now wired as the daemon's default: `AgentDelegatedReviewer` (`packages/core/src/agent-delegated-reviewer.ts`) delegates review to whichever coding agent is connected to the daemon over MCP for the session, instead of calling an external AI provider — no third-party AI provider or credentials are involved. It parks each review request behind `GET /v1/semantic-reviews/pending` and `POST /v1/semantic-reviews/:requestId/submit` (`apps/daemon/src/index.ts`), exposed to the connected agent as the `list_pending_semantic_reviews`/`submit_semantic_review` MCP tools (`apps/mcp-server/src/index.ts`, `docs/mcp-clients.md`); see the Phase 6 entry under section 20 for the exact scope and what's still outstanding.
...
None outstanding for Phase 6 as scoped; see the Phase 6 entry under section 20 for the exact remaining item (auto-triggering a review at classification time rather than on demand).
...
### Phase 6 — AI review and publishing — COMPLETE

- Done: provider-neutral `SemanticReviewRequest`/`SemanticReview`/`SemanticReviewer` types and a strict runtime-validated response schema live in `packages/core` (`packages/core/src/semantic-review.ts`), with zero provider SDK in protocol/transaction code. A redaction-and-bundle-construction function rejects configured exclusions, `.env`/private-key/credential paths, and secret-pattern content, logging only hashes and reasons. Workspace policy (`externalAiReview: disabled|approved`, allowed-provider list, per-review local confirmation) is read from committed `.crosscode/config.yaml`. The daemon (`apps/daemon/src/index.ts`) only offers review for the two classifications deterministic analysis already marks ambiguous (`likely-compatible`, `semantic-overlap`); high/critical risk always forces `requiresHumanApproval`. A review only ever writes an immutable audit record (`semantic_review` table) — accepting it is itself audit-only, and materialization still runs through the existing checkpointed `accept()` path, which re-verifies base/local/proposed hashes against the approved review and refuses on any drift. Rejecting a review (or not approving one) leaves the working tree and Git state untouched. The reviewer is dependency-injected (`DaemonOptions.reviewer`); a `MockSemanticReviewer` test double in `packages/core` remains for unit coverage. Unit/integration coverage: redaction, strict schema handling of malformed output, policy gates, prompt-injection-resistant request construction, human-rejection safety, and secrets never appearing in provider requests or audit logs (`packages/core/src/semantic-review.test.ts`, `apps/daemon/src/semantic-review.integration.test.ts`).
- Done: a real, non-mock `SemanticReviewer` implementation, `AgentDelegatedReviewer` (`packages/core/src/agent-delegated-reviewer.ts`), is now wired as the daemon's default reviewer (`apps/daemon/src/runtime.ts`). It is not an external AI provider — no third-party AI vendor SDK, API key, or network call is involved. Instead it delegates review to whichever coding agent (Claude Code, Codex, etc.) is already connected to the daemon over MCP for the session: `review()` parks the redacted bundle in an in-memory pending map and returns a promise that resolves only when that agent submits an answer, or degrades to the safe `UNCERTAIN_FALLBACK` (`requiresHumanApproval: true`) after `timeoutMs` (default 600000ms) with no response. Two daemon-local HTTP routes expose the pending queue outside the daemon's usual `runExclusive` mutex, deliberately, since a pending review can stay open across the request that created it (`apps/daemon/src/index.ts`): `GET /v1/semantic-reviews/pending` (`AgentDelegatedReviewer.listPending()`) and `POST /v1/semantic-reviews/:requestId/submit` (`AgentDelegatedReviewer.submit()`). These are surfaced to the connected agent as two new MCP tools, `list_pending_semantic_reviews` and `submit_semantic_review` (`apps/mcp-server/src/index.ts`), the latter's input schema built directly from `packages/core`'s `semanticReviewSchema` via `.extend({ requestId: z.string() })` rather than a hand-duplicated JSON Schema, so it cannot drift from the daemon's own validation. Covered by `packages/core/src/agent-delegated-reviewer.test.ts` (5 tests: pending/submit bookkeeping, unknown-requestId rejection, and the timeout fallback), the `"agent-delegated semantic review over the local HTTP API"` suite in `apps/daemon/src/semantic-review.integration.test.ts` (2 tests: a full pending → submit → accept round trip over real HTTP, and the timeout-fallback path), and a new MCP-level end-to-end test in `apps/mcp-server/src/index.test.ts` (`"lets the connected agent discover and resolve a pending semantic review over MCP"`) that drives a real stdio `initialize`/`tools/list`/`tools/call` exchange against a real daemon: it forces a `likely-compatible` proposal into review, calls `list_pending_semantic_reviews` until the bundle appears, calls `submit_semantic_review` with a well-formed review, and asserts both the daemon's HTTP response and `listSemanticReviewsFor()` reflect the submitted `compatible` classification. All three files pass (`pnpm exec vitest run apps/mcp-server/src/index.test.ts apps/daemon/src/semantic-review.integration.test.ts packages/core/src/agent-delegated-reviewer.test.ts`: 3 test files, 22 tests, all passing); `pnpm exec tsc --noEmit` passes with these changes in place.
...
- Done: the daemon now auto-triggers a semantic review the moment an incoming remote proposal is first stored as `status: "proposed"` and deterministic analysis classifies it as review-eligible (`isReviewEligible`: `likely-compatible`/`semantic-overlap`), instead of only on demand. Auto-triggered reviews always use a fixed provider id, `AGENT_DELEGATED_PROVIDER_ID = "agent-delegated"` (`packages/core/src/semantic-review.ts`), labeling them as coming from the default `AgentDelegatedReviewer`; a workspace opts in via the same existing `.crosscode/config.yaml` mechanism as on-demand review (`aiReview.externalAiReview: approved` plus `"agent-delegated"` in `aiReview.allowedProviders`) — no new config schema was added. The new private `LocalDaemon.autoTriggerSemanticReviews()` (`apps/daemon/src/index.ts`) runs at the two points a remote proposal transitions to `"proposed"` — the end of `syncRemote()`'s inbound page loop and the end of `sync()`'s `CoordinationService` path, both right after the existing `autoApplyEligibleProposals()` call — and is a no-op for locally captured transactions. It skips operations no longer `"proposed"` (e.g. already auto-applied) and, per `(operationId, path)`, skips if a review from `"agent-delegated"` already exists (via `listSemanticReviewsFor`), so repeated syncs never duplicate a review. Critically, the actual `reviewer.review()` call — which can take up to 10 minutes for `AgentDelegatedReviewer` to hear back from the connected agent — is never awaited: `requestSemanticReview()` is invoked fire-and-forget (`void ... .catch(() => {})`) so sync/ingestion is never blocked, and a review failure is swallowed since on-demand review remains available regardless. Covered by the `"auto-triggered semantic review on incoming remote proposals"` suite in `apps/daemon/src/semantic-review.integration.test.ts` (4 tests: auto-creation with `providerId === "agent-delegated"` and no explicit call, no auto-trigger when policy disables external review or omits the provider from the allow list, no duplicate review on a repeated trigger for the same operation/path, and `sync()` returning promptly — proving fire-and-forget — against a reviewer double whose `review()` never resolves within the test), plus a unit test in `packages/core/src/semantic-review.test.ts` asserting the exact `AGENT_DELEGATED_PROVIDER_ID` string. `pnpm exec tsc --noEmit` and `pnpm exec vitest run apps/daemon/src/semantic-review.integration.test.ts packages/core/src/semantic-review.test.ts` both pass with these changes in place.

**Exit criteria:** ambiguous changes receive a non-authoritative AI proposal, high-risk changes require approval, and publishing creates/pushes normal Git commits without force operations. Met: `AgentDelegatedReviewer` is a real, non-mock reviewer that produces a genuine (agent-authored, not hardcoded) AI proposal end-to-end over MCP, with `MockSemanticReviewer` retained only for deterministic unit coverage, and review is now auto-triggered at classification time for incoming remote proposals rather than only on demand.
