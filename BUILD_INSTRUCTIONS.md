...
- [ ] A user can keep using ordinary `git commit`, `git pull`, branches, worktrees, and rebase; Crosscode detects and safely reconciles state changes. Branch switches, hard resets, and the duration of an in-progress merge are covered (`apps/daemon/src/index.test.ts`, and a git-reset case in `apps/daemon/src/three-participant.e2e.test.ts`), but there is no fixture that exercises an actual `git rebase` or `git worktree add`/`git pull` by name against a running daemon, even though `discoverRepository` in `packages/git/src/index.ts` already detects a rebase-in-progress via `rebase-merge`/`rebase-apply`; that specific path is unexercised.
