...
- It opens the browser at `${WEB_URL}/auth/cli.html?port=<port>&state=<state>`, where `WEB_URL` comes from `--web <url>`, else `CROSSCODE_WEB_URL`, else the legacy `CROSSCODE_DASHBOARD_URL` the MCP server already reads, else the hosted default `DEFAULT_WEB_URL` in `apps/daemon/src/hosted.ts`. `WEB_URL_REQUIRED` is therefore no longer reachable: bare `crosscode login` targets the hosted site.
