...
describe("MCP daemon boundary", () => {
  it("maps tools through the authenticated HTTP client", async () => {
    ...
    await expect(tools.get_workspace_state()).resolves.toMatchObject({ root: expectedRoot, workspaceId: "w" });
  });
  ...
  it("serves a standards-compliant MCP server over stdio: real initialize, tools/list, and tools/call", async () => {
    ...
    const publishIntentTool = tools.find((tool) => tool.name === "publish_intent")!;
    expect(publishIntentTool.inputSchema.type).toBe("object");
    expect(publishIntentTool.inputSchema.properties).toHaveProperty("intent");

    await writeFile(join(root, "a.txt"), "captured through MCP\n");
    const captured = await client.callTool({ name: "publish_intent", arguments: { intent: "Coordinate through MCP" } });
    const operation = JSON.parse((captured.content as Array<{ type: string; text: string }>)[0]!.text) as { id: string; transaction: { intent: string } };
    expect(operation.transaction.intent).toBe("Coordinate through MCP");
  }, 20_000);
  ...
  it("auto-bootstraps identity and spawns its own daemon when neither exists yet, so a fresh checkout works with zero manual setup", async () => {
    ...
    await writeFile(join(root, "a.txt"), "works without manual setup\n");
    const captured = await client.callTool({ name: "publish_intent", arguments: { intent: "Works without any manual init or daemon start" } });
    const operation = JSON.parse((captured.content as Array<{ type: string; text: string }>)[0]!.text) as { transaction: { intent: string } };
    expect(operation.transaction.intent).toBe("Works without any manual init or daemon start");
  }, 20_000);
});
