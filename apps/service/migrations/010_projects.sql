...
-- workspace. Until now `workspaces` was the only container, so activity could not be
-- attributed to the checkout it came from.
--
...
-- consumers group those under "Unassigned". ON DELETE SET NULL so removing a project
